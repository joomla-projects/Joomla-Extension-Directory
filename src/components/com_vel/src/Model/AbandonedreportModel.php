 ...
 * @since 4.0
 */
class AbandonedreportModel extends ItemModel
{
    /**
     * The item object
     *
    ...
    {
        // Get the id.
        $id = (!empty($id)) ? $id : (int)$this->getState('abandonedreport.id');
    ...
    public function getItem($pk = null): mixed
    {
        ...
        if ($this->item === null) {
            ...
            if (empty($pk)) {
                $pk = $this->getState('abandonedreport.id');
            }
                ...
                if (empty($result) || JedHelper::isAdminOrSuperUser()) {
                    ...
                    if ($published = $this->getState('filter.published')) {
                        if (isset($table->state) && $table->state != $published) {
                            throw new Exception(Text::_('COM_VEL_ITEM_NOT_LOADED'), 403);
                        }
                    }
        ...
        if (!empty($this->item->passed_to_vel) || $this->item->passed_to_vel == 0) {
            $this->item->passed_to_vel = Text::_(
                'COM_VEL_GENERAL_PASSED_TO_VEL_OPTION_' . $this->item->passed_to_vel
            );
        }

        if (!empty($this->item->data_source) || $this->item->data_source == 0) {
            $this->item->data_source = Text::_(
                'COM_VEL_GENERAL_DATA_SOURCE_OPTION_' . $this->item->data_source
            );
        }
