...
class JedCategoryField extends ListField
{
    ...
    private function getCategories(): array
    {
        ...
        $query->select('a.id AS value, a.title AS text, a.level')->from($db->quoteName('#__categories', 'a'))->leftJoin($db->quoteName('#__categories', 'b'), 'a.lft > b.lft AND a.rgt < b.rgt')->where('a.published = 1 AND a.level >= 1 AND a.extension = ' . $db->q($option))->group('a.id, a.title, a.level, a.lft, a.rgt, a.extension, a.parent_id')->order(' a.lft ASC ')->where('a.access IN (' . implode(',', $user->getAuthorisedViewLevels()) . ')');
        $db->setQuery($query);
